def read_card_file(filepath):
    # Open csv and load data from file
    global CardId
    global CardName
    global CardMana
    global CardColor
...
def write_card_data(filepath):
    # If the data has more info than just the header, make a backup.
    if len(CardId) > 1:
        output_text = ""
        i = 0
        while i < len(CardId):
            ...
            i += 1
        card_backup_file = open(filepath, "w")
        card_backup_file.write(output_text)
        card_backup_file.close()
    else:
...
def remove_cards():
    # A loop to allow the user to remove multiple cards, confirming before each card is added.
    global CardId
    global CardName
    global CardMana
    global CardColor
...
def card_search():
    global SearchCardId
    global SearchCardName
    global SearchCardMana
    global SearchCardColor
    global SearchCardType
    global SearchCardRarity
    search_cat_list = []
    no_field_input = True
    ...
    while i < len(search_cat_list):
        if user_input.lower() in search_cat_list[i].lower():
            # cards always have the id of their row; important to note when iterating like this.
            result_id.append(i)
        i += 1

    i = 0
    while i < len(result_id):
        SearchCardId.append(CardId[result_id[i]])
        SearchCardName.append(CardName[result_id[i]])
        SearchCardMana.append(CardMana[result_id[i]])
        SearchCardColor.append(CardColor[result_id[i]])
        SearchCardType.append(CardType[result_id[i]])
        SearchCardRarity.append(CardRarity[result_id[i]])
        i += 1
    print_cards_v2(SearchCardId, SearchCardName, SearchCardMana, SearchCardColor, SearchCardType, SearchCardRarity)
...
readCardFilePath = "testCardDataParallel.csv"
writeCardFilePath = "testCardDataParallel.csv"
